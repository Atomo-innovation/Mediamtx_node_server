...
Usage:
  python3 face_detector.py \\
      ...
      [--fps        10]       # inference frames/sec per camera (real-time)
      [--interval   0.75]     # min seconds between events per tracked face
      [--vote_frames 2]       # frames to confirm (1 for fast known match)
      [--use_npu    true]     # MobileFaceNet on TIM-VX NPU (required by default)
      [--realtime]            # max speed: fps=12 vote=1 interval=0.5
      [--max_faces  16]       # all persons visible in the room
      [--face_thresh 0.50]
...
os.environ.setdefault("TIMVX_CACHE_DIR", "/tmp/timvx_cache")
os.environ.setdefault("VSI_USE_IMAGE_PROCESS", "1")
os.environ.setdefault("OPENCV_DNN_OPENCL", "0")
...
ap.add_argument("--fps",          type=float, default=10.0,
                help="Detection frames/sec per camera (default 10, real-time)")
ap.add_argument("--interval",     type=float, default=0.75,
                help="Min seconds between events per tracked face")
ap.add_argument("--tolerance",    type=float, default=0.45,
                help="Cosine distance threshold for identity match")
ap.add_argument("--vote_frames",  type=int,   default=2,
                help="Frames to confirm identity (default 2 for speed)")
ap.add_argument("--fast_known_conf", type=float, default=0.68,
                help="Enrolled person: confirm in 1 frame if match >= this")
ap.add_argument("--require_npu",  default=True, type=_str2bool,
                help="Exit if TIM-VX NPU recognizer cannot load (no CPU fallback)")
ap.add_argument("--max_faces",    type=int,   default=16,
                help="Max faces processed per frame (all persons in room)")
ap.add_argument("--recog_queue",  type=int,   default=48,
                help="Recognition queue depth per camera")
ap.add_argument("--track_iou",    type=float, default=0.35,
                help="IoU threshold to match face across frames")
ap.add_argument("--track_max_miss", type=int, default=12,
                help="Drop track after N frames without detection")
ap.add_argument("--realtime",     action="store_true",
                help="Preset: fps=12 vote=1 interval=0.5 (max speed)")
...
ap.add_argument("--crop_pad",     type=float, default=0.22,
                help="Padding around face box for display crops")
ap.add_argument("--crop_jpeg_quality", type=int, default=92,
                help="JPEG quality for face thumbnails (1-100)")
ap.add_argument("--crop_max_px",  type=int,   default=640,
                help="Max longest edge for stored face thumbnails")
ap.add_argument("--crop_min_px",  type=int,   default=320,
                help="Upscale small crops to this longest edge for HD thumbnails")
...
if ARGS.realtime:
    ARGS.fps      = max(ARGS.fps, 12.0)
    ARGS.vote_frames = min(ARGS.vote_frames, 1)
    ARGS.interval = min(ARGS.interval, 0.5)
    ARGS.fast_known_conf = max(ARGS.fast_known_conf, 0.65)
...
known_centroids: Dict[str, np.ndarray] = {}   # name → 512-d L2 unit vector
                                               # "__count_name" → int
known_portraits: Dict[str, str] = {}          # name → HD JPEG base64 (display only)
...
def load_db() -> None:
    global known_centroids, known_portraits
    ...
    try:
        ...
        with _db_lock:
            known_centroids = {}
            known_portraits = dict(d.get("portraits") or {})
            if "centroids" in d:
...
def _save_db_locked() -> None:
    ...
    with open(DB_PATH, "w") as f:
        json.dump({
            "centroids": centroids,
            "counts":    counts,
            "names":     [k for k in centroids],
            "portraits": known_portraits,
        }, f)
...
print(f"  Tolerance  : {ARGS.tolerance}   Vote: {ARGS.vote_frames}  Fast≥{ARGS.fast_known_conf}", flush=True)
print(f"  Inference  : {ARGS.fps} fps/cam  Max faces: {ARGS.max_faces}  Track IoU: {ARGS.track_iou}", flush=True)
print("="*56 + "\n", flush=True)
...
try:
    ...
    if ARGS.require_npu:
        print(f"[Boot] FATAL: TIM-VX NPU required but failed: {e}", flush=True)
        sys.exit(1)
...
# FACE CROP HELPERS
# ══════════════════════════════════════════════════════════════════════════════
def _crop_padded(frame: np.ndarray, box: np.ndarray, pad: float = None
                 ) -> Optional[np.ndarray]:
    if pad is None:
        pad = ARGS.crop_pad
    h, w = frame.shape[:2]
...
def _prepare_hd_crop(img: np.ndarray) -> np.ndarray:
    """Resize for HD thumbnails: upscale tiny CCTV crops, cap very large frames."""
    if img is None or img.size == 0:
        return img
    h, w = img.shape[:2]
    long_edge = max(h, w)
    if long_edge < ARGS.crop_min_px:
        scale = ARGS.crop_min_px / long_edge
        nw, nh = max(1, int(w * scale)), max(1, int(h * scale))
        img = cv2.resize(img, (nw, nh), interpolation=cv2.INTER_CUBIC)
        h, w = img.shape[:2]
        long_edge = max(h, w)
    if long_edge > ARGS.crop_max_px:
        scale = ARGS.crop_max_px / long_edge
        nw, nh = max(1, int(w * scale)), max(1, int(h * scale))
        img = cv2.resize(img, (nw, nh), interpolation=cv2.INTER_AREA)
    return img


def _to_b64(img: np.ndarray, quality: int = None) -> str:
    q = ARGS.crop_jpeg_quality if quality is None else quality
    img = _prepare_hd_crop(img)
...
class FaceTracker:
    """Stable track IDs across frames so each person in the room keeps a vote buffer."""

    def __init__(self) -> None:
        self._tracks: Dict[int, dict] = {}
        self._next_id = 0

    def assign(self, boxes: np.ndarray) -> List[int]:
        if boxes is None or len(boxes) == 0:
            for tid in list(self._tracks):
                self._tracks[tid]["miss"] += 1
                if self._tracks[tid]["miss"] > ARGS.track_max_miss:
        ...
        for i in range(n):
            box = boxes[i]
            best_tid, best_iou = None, ARGS.track_iou
            for tid in track_ids:
                if tid in used:
                    continue
                iou = _box_iou(box, self._tracks[tid]["box"])
                if iou > best_iou:
                    best_iou, best_tid = iou, tid
            if best_tid is not None:
                out[i] = best_tid
                used.add(best_tid)
                self._tracks[best_tid]["box"] = box.astype(np.float32)
                self._tracks[best_tid]["miss"] = 0
            else:
                tid = self._next_id
                self._next_id += 1
                out[i] = tid
                self._tracks[tid] = {"box": box.astype(np.float32), "miss": 0}
...
class RecogWorker(threading.Thread):
    ...
    def run(self) -> None:
        ...
        while self._running:
            ...
            # ── voting (fast path for enrolled persons on NPU) ───────────────
            ...
            peak_conf   = max((v[1] for v in votes if v[0] == most_common), default=0.0)

            need = 1 if (most_common != "Unknown"
                         and peak_conf >= ARGS.fast_known_conf) else ARGS.vote_frames
            if len(votes) < need:
                continue
            if need > 1 and vote_count < max(2, need // 2 + 1):
                continue
...
class CameraWorker(threading.Thread):
    ...
    def __init__(self, cam: dict, event_q: queue.Queue):
        ...
        self._recog_q  = queue.Queue(maxsize=max(8, ARGS.recog_queue))
    ...
    def run(self) -> None:
        ...
        try:
            while self._run:
                ...
                # Process every face in frame (highest confidence first)
                order = np.argsort(scores)[::-1][: max(1, ARGS.max_faces)]
                track_ids = self._tracker.assign(boxes_px[order])
                ...
                for rank, idx in enumerate(order):
                    ...
                    tid = track_ids[rank] if rank < len(track_ids) else rank
                    pos_key = f"t{tid}"
...
class _MgmtHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self) -> None:
        if self.path == "/api/known":
            with _db_lock:
                portraits = dict(known_portraits)
            self._send_json(200, {"names": _known_names(), "portraits": portraits})
    ...
    def do_POST(self) -> None:
        ...
        portrait_b64 = _to_b64(img)
        with _db_lock:
            if portrait_b64:
                known_portraits[name] = portrait_b64
            _save_db_locked()
